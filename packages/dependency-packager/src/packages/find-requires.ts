...
function rewritePath(path: string, currentPath: string) {
    ...
    try {
      if (requirePath.startsWith('glob:')) {
        ...
        newPaths = files
          ...
          .map(p => rewritePath(p, filePath));
      } else {
        newPaths = [rewritePath(requirePath, filePath)];
      }
    } catch (e) {
...
export default async function findRequires(
  ...
  // to download.
  return packageName === 'node-libs-browser'
  || (sizeMB > 8
